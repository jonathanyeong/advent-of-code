...
def parse_input(input)
  ...
  input.each do |i|
    ...
    range = parsed_i[0].split("-")
    character = parsed_i[1].delete_suffix(":")
    password = parsed_i[2]
    parsed_inputs << { min: range[0].to_i, max: range[1].to_i, character: character, password: password}
  end
  parsed_inputs
end

parsed_inputs = parse_input(input)
valid_passwords = 0
parsed_inputs.each do |p|
  valid_passwords += 1 if valid_password?(p[:min], p[:max], p[:character], p[:password])
end
