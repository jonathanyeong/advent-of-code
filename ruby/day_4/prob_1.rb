small_input = "ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
...
def parse_input(input)
  ...
  input.split("\n").each do |line|
    if line == ""
      passports << Passport.new(passport["byr"], passport["iyr"], passport["eyr"], passport["hgt"], passport["hcl"], passport["ecl"], passport["pid"], passport["cid"])
      passport = {}
      next
    end
    ...
    if line.count(" ") == 0
      ...
      line.split(" ").each do |vals|
        passport[vals.split(":")[0]] = vals.split(":")[1]
      end
    end
  end
  passports << Passport.new(passport["byr"], passport["iyr"], passport["eyr"], passport["hgt"], passport["hcl"], passport["ecl"], passport["pid"], passport["cid"]) unless passport.empty?
  passports
end
